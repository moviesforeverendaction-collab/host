...
# ── Start Docker daemon pointing data-root at persistent storage ──────────────
echo "[entrypoint] Starting Docker daemon (data-root=$DOCKER_DATA)..."
dockerd \
...
# Wait for Docker socket to be ready (up to 60 s)
...
# ── Pre-build the VPS base image if not already in cache ─────────────────────
if ! docker image inspect telegram-bot-vps-base > /dev/null 2>&1; then
    echo "[entrypoint] Building VPS base image (first boot, this takes ~2 min)..."
    docker build \
        -t telegram-bot-vps-base \
        -f /app/Dockerfile.vps \
        /app \
        > "$LOG_DIR/vps-image-build.log" 2>&1 \
    && echo "[entrypoint] VPS base image built successfully." \
    || echo "[entrypoint] WARNING: VPS base image build failed — check $LOG_DIR/vps-image-build.log"
else
    echo "[entrypoint] VPS base image already exists — skipping build."
fi
